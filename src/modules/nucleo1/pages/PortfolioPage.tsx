...
import { projects, helpers, labels, clientes, nucleos } from "../mockData";
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Filtros por cliente, núcleo, gestor, status e prioridade. Score de saúde por projeto."
        actions={
          <>
            <Button
              variant={view === "cards" ? "primary" : "outline"} size="sm"
              onClick={() => setView("cards")}
            ><LayoutGrid /> Cards</Button>
            <Button
              variant={view === "table" ? "primary" : "outline"} size="sm"
        ...
        <div className="border-2 border-foreground bg-card p-4 shadow-brutal-sm flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <Input
              ...
              className="pl-9 border-2 border-foreground rounded-none h-10"
            />
          </div>
          <Selector value={cliente} onChange={setCliente} placeholder="Cliente"
            options={clientes.map((c) => ({ v: c.id, l: c.nome }))} />
          <Selector value={nucleo} onChange={setNucleo} placeholder="Núcleo"
            options={nucleos.map((n) => ({ v: n.id, l: n.nome }))} />
          <Selector value={status} onChange={setStatus} placeholder="Status"
            options={Object.entries(labels.status).map(([v, l]) => ({ v, l }))} />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((p) => (
              <Link
                ...
                className="border-2 border-foreground bg-card p-5 shadow-brutal-sm hover-brutal block"
              >
                ...
                <h3 className="font-display text-lg leading-tight mb-2">{p.nome}</h3>
                ...
                <div className="text-xs text-muted-foreground mb-3">
                  <strong>{helpers.getCliente(p.clienteId)?.nome}</strong> · {helpers.getNucleo(p.nucleoId)?.nome}
                </div>
              ...
              <tbody className="divide-y-2 divide-foreground">
                {filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-muted/40">
                    ...
                    <td>
                      <Link to={`/projetos-squads/app/projeto/${p.id}`} className="font-bold hover:underline">{p.nome}</Link>
                    </td>
                    <td className="text-xs">{helpers.getCliente(p.clienteId)?.nome}</td>
                    <td className="text-xs">{helpers.getNucleo(p.nucleoId)?.nome}</td>
                    <td><StatusBadge status={p.status} /></td>
                    <td><PriorityChip p={p.prioridade} /></td>
                    <td><HealthDot saude={p.saude} withLabel /></td>
                    <td className="text-right p-3 font-mono text-xs">{p.progresso}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const Selector = ({
  ...
  <select
    ...
    className={cn(
      "h-10 border-2 border-foreground bg-background px-3 text-xs font-bold uppercase tracking-wide rounded-none",
    )}
  >
